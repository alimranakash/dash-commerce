...
/**
 * One bucket per row, not one per property: a checkout is filed by the furthest
 * thing that happened to it, so a failed attempt the seller has since rung
 * counts as contacted rather than showing up under both.
 */
export type IncompleteOrderFilterKey =
  | "all"
  | "failed"
  | "started"
  | "contacted"
...
type IncompleteOrderListControlsProps = {
  activeFilter: IncompleteOrderFilterKey;
  counts: Record<Exclude<IncompleteOrderFilterKey, "clean">, number>;
  dateRange: string;
  search: string;
};

const tabs: Array<{ badgeClass?: string; key: IncompleteOrderFilterKey; label: string }> = [
  { badgeClass: "bg-[#f0f0f3] text-[#555762]", key: "all", label: "All" },
  { badgeClass: "bg-[#ffe8ed] text-[#f05268]", key: "failed", label: "Failed" },
  { badgeClass: "bg-[#fff1df] text-[#a7650c]", key: "started", label: "Not Submitted" },
...
export function IncompleteOrderListControls({
  activeFilter,
  counts,
  dateRange,
  search
}: IncompleteOrderListControlsProps) {
  return (
    <section className="rounded-xl border border-[#ececf5] bg-white px-6 py-5 shadow-[0_8px_24px_rgba(62,54,114,0.04)]">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
        <nav
          aria-label="Incomplete order filters"
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link
                ...
                href={`/dashboard/orders/incomplete${params.size ? `?${params.toString()}` : ""}`}
                key={tab.key}
              >
                {tab.label}
                {tab.badgeClass ? (
                  ...
                  >
                    {counts[tab.key as Exclude<IncompleteOrderFilterKey, "clean">]}
                  </span>
                ) : null}
              </Link>
            );
        ...
        <DashboardQueryForm
          actionPath="/dashboard/orders/incomplete"
        ...
        >
          ...
          />
          <input
            aria-label="Incomplete order date range"
