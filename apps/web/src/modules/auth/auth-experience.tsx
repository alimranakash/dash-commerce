  ...
  return (
    <main className={styles.authPage}>
      <section className={styles.formSide}>
        <Link className={styles.brand} href="/"><span>D</span><b>Dash Commerce <i>OS</i></b></Link>
        <div className={styles.formFrame}>
          <div className={styles.intro}><span>{eyebrow}</span><h1>{title}</h1><p>{description}</p></div>
          {children}
        </div>
        <footer><span>© 2026 Dash Commerce OS</span><Link href="/">Back to homepage <ArrowUpRight /></Link></footer>
      </section>

      <aside className={styles.showcase} aria-label="Dash Commerce OS product preview">
        ...
        <div className={styles.previewStage}>
          <div className={styles.dashboardPreview}>
            <aside><b>D</b>{[BarChart3, Box, ReceiptText].map((Icon, index) => <span data-active={index === 0} key={index}><Icon /></span>)}</aside>
          ...
          <div className={styles.aiCard}><header><span><Bot /></span><div><b>StoreOS AI</b><small>Live store context</small></div><i /></header><p>Revenue is up 18% this week. Three products need your attention.</p><footer><Check /> Insight ready</footer></div>
