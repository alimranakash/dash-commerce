...
export function FashionStorefrontHeader({
  ...
  const shellStyle = {
    ...
    "--fashion-header-height": `${Math.max(68, Math.min(settings.header.height, 96))}px`
  } as CSSProperties;
  ...
  return (
    <div
      className={`fashion-header-shell${overlay ? " fashion-header-shell-overlay" : ""}${announcementEnabled ? " fashion-header-shell-has-announcement" : ""}`}
      ...
      {announcementEnabled ? (
        <div className="fashion-announcement">
          <div className="fashion-announcement-message">
            {resolveAnnouncement(messages, announcementText, homeHref)}
          </div>
...
function resolveAnnouncement(
  ...
  const message = messages[0] ?? (fallbackText ? { text: fallbackText } : null);
