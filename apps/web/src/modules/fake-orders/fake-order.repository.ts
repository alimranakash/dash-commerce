 ...
 * through the digits-only expression index, and the four counts the rules need
 * come back as counts. `recentOrderCount` stays correlated to each order's own
 * `createdAt`, which is why this is a join rather than a plain GROUP BY.
 */
export async function getPhoneRiskSignals(storeId: string, orderIds: string[]) {
  ...
  return prisma.$queryRawUnsafe<PhoneRiskSignalRow[]>(
    `
    WITH target AS (
      SELECT "id", "createdAt", regexp_replace("customerPhone", '\\D', '', 'g') AS phone
      FROM ${orderTable()}
