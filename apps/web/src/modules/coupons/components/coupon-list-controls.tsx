...
export function CouponListControls({ activeFilter, dateRange, search }: CouponListControlsProps) {
  return (
    <section className="flex min-h-[580px] flex-col rounded-xl border border-[#ececf5] bg-white px-6 py-6 shadow-[0_8px_24px_rgba(62,54,114,0.04)]">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
        <nav aria-label="Coupon filters" className="-mb-px flex min-w-0 gap-5 overflow-x-auto border-b border-[#eeeef5]">
          {tabs.map((tab) => {
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-4 py-16 text-center">
        <div className="mb-6 grid h-32 w-32 place-items-center rounded-2xl bg-[#f3efff] text-[#7950f2]">
          <FileText aria-hidden="true" className="h-24 w-24" fill="#8b5cf6" stroke="white" strokeWidth={1.5} />
        </div>
        <h2 className="m-0 text-xl font-semibold text-[#20212a]">No coupons found</h2>
        <p className="mt-4 max-w-sm text-sm leading-6 text-[#85869a]">All type of Coupons activities will appear here once they occur.</p>
        <Link className="mt-5 inline-flex items-center gap-1.5 rounded-lg border border-[#7c3aed] px-4 py-2.5 text-sm font-medium text-[#6d3cf5] hover:bg-[#f7f3ff]" href="/dashboard/coupons/new">
