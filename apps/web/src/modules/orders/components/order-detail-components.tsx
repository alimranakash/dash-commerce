...
export function ShippingCard({ cost, deliveryStatus, method }: { cost: string; deliveryStatus: string; method: string }) {
  return <DetailCard icon={Truck} title="Shipping"><InfoRows rows={[["Courier", "Not booked"], ["Shipping Method", method], ["Tracking Number", "Not assigned"], ["Delivery Status", <OrderStatusBadge key="status" status={deliveryStatus} />], ["Shipping Cost", cost]]} /></DetailCard>;
}
...
export function QuickActionsCard({ orderId }: { orderId: string }) { return <DetailCard title="Quick Actions"><OrderQuickActions orderId={orderId} /></DetailCard>; }
