...
const socialFields = [
  { icon: MessageSquare, label: "Facebook Page URL", name: "facebookUrl", placeholder: "https://facebook.com/your-store", type: "url" },
  { icon: Camera, label: "Instagram URL", name: "instagramUrl", placeholder: "https://instagram.com/your-store", type: "url" },
  { icon: AtSign, label: "X / Twitter URL", name: "twitterUrl", placeholder: "https://x.com/your-store", type: "url" },
  { icon: Play, label: "YouTube URL", name: "youtubeUrl", placeholder: "https://youtube.com/@your-store", type: "url" },
  { icon: Music2, label: "TikTok URL", name: "tiktokUrl", placeholder: "https://tiktok.com/@your-store", type: "url" },
  { icon: Briefcase, label: "LinkedIn URL", name: "linkedinUrl", placeholder: "https://linkedin.com/company/your-store", type: "url" },
  { icon: MessageCircle, label: "WhatsApp Number", name: "whatsappNumber", placeholder: "+880 1XXX-XXXXXX", type: "tel" }
] as const;
  ...
  return (
    <form className="grid gap-5" onSubmit={submitSettings} ref={formRef}>
      ...
      <div className="grid items-start gap-5 xl:grid-cols-2">
        ...
        <SettingsCard icon={Share2} title="Social Profiles">
          <div className="grid gap-4 sm:grid-cols-2">
            {socialFields.map((field) => {
              const Icon = field.icon;
              return (
                <SocialField error={errors[field.name]} key={field.name} label={field.label}>
                  <SocialIconInput icon={Icon} name={field.name} placeholder={field.placeholder} type={field.type} />
                </SocialField>
              );
            })}
          </div>
      ...
      <div className="flex flex-wrap justify-end gap-3">
        ...
        <button className="inline-flex h-11 items-center gap-2 rounded-lg bg-[#7548f5] px-5 text-sm font-semibold text-white shadow-sm hover:bg-[#6436e8]" type="submit"><Save className="h-4 w-4" />Save Settings</button>
      </div>
    </form>
  );
}
...
function SocialIconInput({ icon: Icon, ...props }: InputHTMLAttributes<HTMLInputElement> & { icon: ComponentType<{ className?: string }> }) {
  return (
    <div className="flex h-12 w-full overflow-hidden rounded-lg border border-[#dedcea] bg-white focus-within:border-[#8b5cf6] focus-within:ring-2 focus-within:ring-[#7c3aed]/10">
      <span className="grid min-w-12 shrink-0 place-items-center border-r border-[#e5e3f1] bg-[#fafaff] text-[#7650e8]"><Icon className="h-4 w-4" /></span>
      <input
        {...props}
        className={`h-full min-w-0 flex-1 border-0 bg-white px-3.5 text-sm font-normal text-[#292a34] outline-none placeholder:text-[#a2a3b0] ${props.className ?? ""}`}
      />
    </div>
  );
}
...
function validateSettings(formData: FormData, enabled: Record<ProviderKey, boolean>) {
  ...
  for (const field of socialFields.filter((entry) => entry.type === "url")) {
    const entry = value(formData, field.name);
    if (entry && !isValidUrl(entry)) errors[field.name] = "Enter a valid URL including https://";
  }

  const whatsapp = value(formData, "whatsappNumber");
  if (whatsapp && !/^\+?[0-9 ()-]{7,24}$/.test(whatsapp)) errors.whatsappNumber = "Enter a valid WhatsApp number.";
...
function isValidUrl(valueToCheck: string) {
  try {
