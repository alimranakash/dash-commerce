...
type BillingDashboardProps = {
  ...
  usage: {
    ...
    products: number;
    staff: number;
    /** Members plus pending invites, split apart for the seat bar's footnote. */
    staffSeats: {
      limit: number | null;
      members: number;
  ...
  return (
    <div className="grid gap-5">
      {state.status === "error" && state.message ? (
        <p className="m-0 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">
          {state.message}
        </p>
      ) : null}
      ...
      <section className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <BillingCard icon={<WalletCards className="h-5 w-5" />} title="Current Plan">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Info
              ...
              value={
                subscription.trialRemaining === null
                  ? "No active trial"
                  : `${subscription.trialRemaining} days`
              }
            />
        ...
        <BillingCard icon={<CheckCircle2 className="h-5 w-5" />} title="Usage">
          <div className="grid gap-4">
            <UsageBar
              label="Products"
              limit={currentPlan?.productLimit ?? 0}
              value={usage.products}
            />
            <UsageBar
              label="Orders (this month)"
              limit={currentPlan?.orderLimit ?? 0}
              value={usage.orders}
            />
            <UsageBar
              footnote={describeSeats(usage.staffSeats)}
              label="Staff"
              limit={currentPlan?.staffLimit ?? 0}
              value={usage.staff}
            />
            ...
            <UsageBar
              label="AI Usage"
              limit={currentPlan?.aiEnabled ? 1000 : 0}
              value={usage.aiUsage}
            />
          </div>
        </BillingCard>
      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_420px]">
        <BillingCard icon={<CreditCard className="h-5 w-5" />} title="Upgrade Plan">
          <div className="mb-4 inline-flex rounded-xl border border-[#e2def6] bg-[#f8f7ff] p-1">
            <button
              className={`h-9 cursor-pointer rounded-lg px-4 text-xs font-semibold ${billingCycle === "MONTHLY" ? "bg-white text-[#6d3cf5] shadow-sm" : "text-[#66677a]"}`}
            ...
            <button
              className={`h-9 cursor-pointer rounded-lg px-4 text-xs font-semibold ${billingCycle === "YEARLY" ? "bg-white text-[#6d3cf5] shadow-sm" : "text-[#66677a]"}`}
            ...
            {plans.map((plan) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  {plan.isFeatured ? (
                    <span className="rounded-full bg-[#f0ebff] px-2 py-1 text-[10px] font-bold text-[#6d3cf5]">
                      Popular
                    </span>
                  ) : null}
                </div>
                <p className="mt-2 min-h-10 text-xs leading-5 text-[#74758a]">
                  {plan.description ?? "A clean plan for growing stores."}
                </p>
                <strong className="mt-4 block text-2xl text-[#20212c]">
                  {plan.currency}{" "}
                  {formatMoney(billingCycle === "YEARLY" ? plan.priceYearly : plan.priceMonthly)}
                </strong>
                <div className="mt-4 grid gap-1 text-xs text-[#626370]">
                  ...
                  <span>
                    {plan.aiEnabled ? "AI Assistant included" : "AI Assistant not included"}
                  </span>
                </div>
              </button>
            ))}
        ...
        <BillingCard icon={<ReceiptText className="h-5 w-5" />} title="Manual Payment">
          <div className="grid gap-3 rounded-xl border border-[#ececf5] bg-[#fbfaff] p-4 text-xs text-[#565762]">
            <PaymentMethod
              label="bKash"
              number={billingSettings.bkashNumber}
              type={billingSettings.bkashAccountType}
            />
            <PaymentMethod
              label="Nagad"
              number={billingSettings.nagadNumber}
              type={billingSettings.nagadAccountType}
            />
            <PaymentMethod
              label="Rocket"
              number={billingSettings.rocketNumber}
              type={billingSettings.rocketAccountType}
            />
            {billingSettings.bankAccountDetails ? (
              <p className="m-0 whitespace-pre-wrap">
                <b>Bank:</b> {billingSettings.bankAccountDetails}
              </p>
            ) : null}
            <p className="m-0 leading-5">
              {billingSettings.paymentInstructions ??
          ...
          <form action={formAction} className="mt-5 grid gap-4" ref={formRef}>
            ...
            <Button
              className="h-11 cursor-pointer rounded-lg bg-[#7c3aed] font-semibold text-white hover:bg-[#6d28d9] disabled:opacity-60"
              disabled={isPending}
              type="submit"
            >
              {isPending
      ...
      <BillingCard icon={<RotateCcw className="h-5 w-5" />} title="Subscription Settings">
        <p className="m-0 text-sm leading-6 text-[#74758a]">
          Manual billing is active for this workspace. Auto-renewal is shown for subscription
          tracking only; payments require admin verification before a plan becomes active.
        </p>
      </BillingCard>
    </div>
  );
...
const inputClass =
  "h-11 w-full rounded-lg border border-[#dedcea] bg-white px-3.5 text-sm font-normal text-[#2d2e38] outline-none placeholder:text-[#a2a3b0] focus:border-[#8b5cf6] focus:ring-4 focus:ring-[#7c3aed]/10";
  ...
  return (
    <section className="rounded-2xl border border-[#ececf5] bg-white p-5 shadow-[0_10px_30px_rgba(62,54,114,0.05)]">
      <header className="mb-5 flex items-center gap-3">
        <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#f0ebff] text-[#6d3cf5]">
          {icon}
        </span>
...
function UsageBar({
  ...
  const full = !unlimited && value >= limit;

  return (
    <div>
      <div className="mb-1.5 flex justify-between text-xs">
        <span className="font-semibold text-[#30313d]">{label}</span>
        <span className="text-[#74758a]">
          {value}
          {suffix} / {unlimited ? "Unlimited" : `${limit}${suffix}`}
        </span>
      </div>
      <div className="h-2.5 overflow-hidden rounded-full bg-[#f0eefb]">
        <div
          className={`h-full rounded-full ${full ? "bg-[#e0803a]" : "bg-[#7c3aed]"}`}
          style={{ width: `${percent}%` }}
        />
      </div>
      {footnote ? (
        <p className="m-0 mt-1.5 text-[11px] leading-4 text-[#8a8b99]">{footnote}</p>
      ) : null}
    </div>
  );
}
...
/**
 * The seat bar's one-line explanation. A full bar over a one-person team reads
 * as a bug unless it says a pending invite is holding the other seat, so the two
 * numbers are named rather than silently summed.
 */
function describeSeats(seats: BillingDashboardProps["usage"]["staffSeats"]): ReactNode {
  const people = `${seats.members} ${seats.members === 1 ? "person" : "people"}`;
  const waiting =
    seats.pendingInvites > 0
      ? `, ${seats.pendingInvites} invite${seats.pendingInvites === 1 ? "" : "s"} waiting`
      : "";
  const manageTeam = (
    <a className="font-semibold text-[#6d3cf5] underline" href="/dashboard/settings/team">
      Manage team
    </a>
  );
  ...
  if (seats.remaining === 0) {
    return (
      <>
        {people}
        {waiting} — every seat is taken. {manageTeam}, or pick a bigger plan below.
      </>
    );
  }
...
function PaymentMethod({
  ...
  return (
    <p className="m-0">
      <b>{label}:</b> {number ?? "Not set"} {type ? `(${type})` : ""}
    </p>
  );
}
  ...
  return (
    <BillingCard icon={<FileText className="h-5 w-5" />} title={title}>
      {payments.length ? (
                ...
                <tr key={payment.invoiceNumber}>
                  ...
                  {invoices ? (
                    <td className="p-3 text-right">
                      <button
                        className="cursor-pointer text-xs font-semibold text-[#6d3cf5]"
        ...
        <div className="rounded-xl border border-dashed border-[#dedcf0] bg-[#fbfaff] p-8 text-center text-sm text-[#74758a]">
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-semibold ring-1 ${tone}`}
    >
      {titleCase(status)}
    </span>
  );
...
function FieldError({
  ...
  return fieldErrors?.[name] ? (
    <span className="text-xs font-medium text-red-600">{fieldErrors[name]}</span>
  ) : null;
}
