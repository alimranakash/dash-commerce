...
import {
  ...
  revealAiApiKeyAction,
  revokeAiApiKeyAction
} from "../../../../modules/ai/ai-key.actions";
 ...
 * stopped working, but only a manager may mint one, read one back, or remove
 * one. The actions re-check that for themselves.
 */
      ...
      <section className="resource-page max-w-none">
        <div>
          ...
          <p className="mt-2 text-sm text-[#737582]">
            ...
            you let it. Keys are read-only, you choose what each one can see, and you can open,
            revoke or delete any of them at any time.
          </p>
          {!access.canManage ? (
            <p className="mt-3 rounded-lg border border-[#f4e6d8] bg-[#fffaf4] px-4 py-3 text-sm text-[#8a6134]">
              You can see which keys exist, but only the store owner or an admin can create, read or
              remove one.
            </p>
          ) : null}
        </div>
        ...
        <AiIntegrationSettings
          ...
          deleteAction={deleteAiApiKeyAction}
          ...
          keys={keys}
          revealAction={revealAiApiKeyAction}
