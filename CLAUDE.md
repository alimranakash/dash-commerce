...
- **Dark mode is generated, not refactored, and that is the whole safety argument.** The dashboard, admin console and storefront templates were written against literal light colours — 3,300 Tailwind arbitrary values in TSX and 1,500 declarations in [globals.css](apps/web/src/app/globals.css) — so [scripts/generate-dark-mode-css.mjs](scripts/generate-dark-mode-css.mjs) *maps* each one to a dark counterpart and writes [dark-mode.generated.css](apps/web/src/app/dark-mode.generated.css) plus a marked block inside each `*.module.css` (a module's class names are hashed, so it cannot be reached from `globals.css`). Every rule it emits is scoped to `[data-theme="dark"]`: with no attribute on `<html>` none of it can match, which is what makes adding a theme to a fully-coloured app additive rather than a rewrite. The mapping is role-aware — a colour becomes a surface, a foreground or a hairline depending on the CSS property or the utility prefix, so `#ececf5` can be a panel in one rule and a rule line in the next — and it decides on **chroma, never HSL saturation**, because `#f5f6ff` is *fully saturated* by that measure and came out a vivid blue until it did. Three things it deliberately will not do: repaint a brand or status colour (the hue is the shop's, only the lightness is the theme's), invert near-whites by lightness (that puts cards *under* the page rather than above it, which is the one thing a dark UI cannot get wrong), or move white text that is sitting on a coloured fill. Hand-written exceptions and the palette live in [dark-mode.css](apps/web/src/app/dark-mode.css), imported **after** the generated half so a deliberate decision beats a mapped one. Merchant colours arrive as inline `style` custom properties and so cannot be re-declared at all — the *consumers* of `--sf-header-bg`, `--store-bg` and `--store-text` are rewritten instead, while `--store-primary` and `--sf-primary` are left exactly as the seller chose them. The preference lives in [modules/theme-mode/](apps/web/src/modules/theme-mode/): `light` (the default, so nothing changes for a seller or shopper who never asks), `dark`, and `system` — resolved to one of the other two *before* it reaches the DOM, so every stylesheet answers one question instead of also duplicating itself under a media query. Add a colour, then run `npm run theme:dark`; `npm run verify:dark-mode` fails if you forget, and fails louder if a rule escapes the dark scope.
